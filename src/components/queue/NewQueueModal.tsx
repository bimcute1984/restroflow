    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 modal-overlay" onClick={onClose}>
      <div className="modal-content w-full sm:max-w-sm overflow-hidden animate-slide-up rounded-t-2xl sm:rounded-2xl" onClick={(e) => e.stopPropagation()}>
        <div style={{ borderBottom: "1px solid var(--border)" }} className="p-4 md:p-5 flex items-center justify-between">
          <h2 style={{ color: "var(--text-primary)" }} className="font-bold text-base md:text-lg">🪑 {t.queue.newQueueTitle}</h2>
        ...
        <div className="flex justify-center py-5 md:py-6">
          <div style={{ background: "var(--yellow-bg)", border: "2px solid var(--yellow-border)" }} className="w-20 h-20 md:w-24 md:h-24 rounded-2xl flex flex-col items-center justify-center gap-1">
            <span style={{ color: "var(--text-muted)" }} className="text-[10px] md:text-xs">{t.queue.queueNumber}</span>
        ...
        <div className="px-4 pb-4 md:px-5 md:pb-5 flex flex-col gap-4">
          <div className="flex flex-col gap-1.5">
            <label style={{ color: "var(--text-muted)" }} className="text-xs font-semibold">{t.queue.customerName} <span style={{ color: "var(--red)" }}>*</span></label>
            <input value={name} onChange={(e) => setName(e.target.value)} placeholder={t.queue.customerNamePlaceholder} autoFocus className="input-styled w-full py-2.5" />
          </div>
          <div className="flex flex-col gap-1.5">
            <label style={{ color: "var(--text-muted)" }} className="text-xs font-semibold">{t.queue.partySize}</label>
          ...
          <div className="flex flex-col gap-1.5">
            <label style={{ color: "var(--text-muted)" }} className="text-xs font-semibold">{t.queue.phone}</label>
          ...
          <button onClick={handleSubmit} disabled={!name.trim()}
            ...
            {t.queue.acceptQueue} #{nextNumber}
          </button>
        </div>
      </div>
    </div>
