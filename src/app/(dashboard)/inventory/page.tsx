...
export default function InventoryPage() {
  ...
  const lowCount = mockInventory.filter((i) => i.currentStock < i.minStock).length;
  ...
  return (
    <div className="p-6 flex flex-col gap-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          { label: "รายการทั้งหมด", value: mockInventory.length, color: "#56b6c2" },
          { label: "สต๊อกต่ำ ⚠️", value: lowCount, color: "#e06c75" },
          { label: "สต๊อกปกติ", value: mockInventory.length - lowCount, color: "#98c379" },
        ...
        {items.map((item, idx) => {
          ...
          return (
            <div
              key={item.id}
              style={{
                borderBottom: idx < items.length - 1 ? "1px solid #2d3141" : undefined,
